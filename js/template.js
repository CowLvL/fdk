...
function getInvites(){
 $.ajax({
  url: '/engine/team-invite-checker.php',
  type: 'post',
  datatype: 'text',
  success: function(data){
    // Perform operation on return value
    $("#team-invites").html(data);
    if($("[id^=invite-block]").length < 2) {
		$("#team-invites").height($("[id^=invite-block]").length*105);
    	$(".slimScrollDiv").height($("[id^=invite-block]").length*105);
    } else {
    	$("#team-invites").height(210);
    	$(".slimScrollDiv").height(210);
    }
    
    if($("[id^=invite-block]").length != 1) {
      $("#team-invites-num").html("Du har " + $("[id^=invite-block]").length + " invitationer");
    } else {
      $("#team-invites-num").html("Du har " + $("[id^=invite-block]").length + " invitation");
    }
    if($("[id^=invite-block]").length > 0) { 
      $("#team-invites-inbox").addClass("fa fa-inbox faa-tada animated");
      $("#team-invites-dropdown").addClass("dropdown-toggle after");
    }
...
// Team invite accepter
function inviteResponse(response, team_id){
  $.ajax({
      type: "POST",
      url: "/engine/team-invite-response.php",
      data: { 
          response: response,
          team_id: team_id 
      }, 
      success: function(result) {
        $("#invite-block-"+result).remove();
        $(".slimScrollDiv").height($(".slimScrollDiv").height() - 105);
        if($("[id^=invite-block]").length != 1) {
          $("#team-invites-num").html("Du har " + $("[id^=invite-block]").length + " invitationer");
        } else {
          $("#team-invites-num").html("Du har " + $("[id^=invite-block]").length + " invitation");
        ...
        if($("[id^=invite-block]").length > 0) { 
          ...
          $("#team-invites-inbox").removeClass("fa fa-inbox faa-tada animated").addClass("fa fa-inbox");
          $("#team-invites-dropdown").removeClass("dropdown-toggle after").addClass("dropdown-toggle");
        }
...
// On click set username
function selectUsername(val) {
  $("input.form-control.swal-user-name").val(val);
...
$(function() {
  ...
  Layout.prototype.fix = function () {
    ...
    var loginboxHeight = $(Selector.loginbox).height() || 0
    // Set the min-height of the content and sidebar based on
    // the height of the document.
    if ($('body').hasClass(ClassName.fixed)) {
      $(Selector.contentWrapper).css('min-height', windowHeight - footerHeight)
	} else if ($('body').hasClass(ClassName.loginPage)) {
      $(Selector.loginbox).css('height', windowHeight)
    } else {
      var postSetHeight
